...
object StunCommandReader {
    ...
    private fun readHeader(input: InputStream): StunHeader {
        ...
        val type = HeaderType.values().find { it.value == typeValue } ?: HeaderType.ERROR
    ...
    private fun readBody(input: InputStream, length: Int): List<StunAttribute> {
        ...
        while (length > index) {
            ...
            val type = AttributeType.values().find { it.value == typeValue } ?: AttributeType.UNKNOWN_ATTRIBUTES
