 ...
 *
 * MultiCamera2 multiCamera2 = new MultiCamera2(openGlView, new ConnectChecker[]{ stream1, stream2 },
 * new ConnectChecker[]{ stream3 });
 *
 * You can set an empty array or null if you don't want use a protocol
 * new MultiCamera2(openGlView, new ConnectChecker[]{ stream1, stream2 }, null); //RTSP protocol is not used
 *
 ...
 *
 * multiCamera2.startStream(MultiType.RTMP, 1, endpoint); //stream2 is started
 * multiCamera2.stopStream(MultiType.RTSP, 0); //stream3 is stopped
 * multiCamera2.getStreamClient(MultiType.RTMP, 0).retry(delay, reason, backupUrl) //retry stream1
 *
 * NOTE:
 * If you call this methods nothing is executed:
 *
 * multiCamera2.startStream(endpoint);
 * multiCamera2.stopStream();
 *
...
class MultiCamera2 : Camera2Base {
