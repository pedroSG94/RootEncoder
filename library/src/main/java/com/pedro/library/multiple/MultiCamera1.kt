 ...
 *
 * MultiCamera1 multiCamera1 = new MultiCamera1(surfaceView, new ConnectChecker[]{ stream1, stream2 },
 * new ConnectChecker[]{ stream3 });
 *
 * You can set an empty array or null if you don't want use a protocol
 * new MultiCamera1(surfaceView, new ConnectChecker[]{ stream1, stream2 }, null); //RTSP protocol is not used
 *
 ...
 *
 * multiCamera1.startStream(MultiType.RTMP, 1, endpoint); //stream2 is started
 * multiCamera1.stopStream(MultiType.RTSP, 0); //stream3 is stopped
 * multiCamera1.getStreamClient(MultiType.RTMP, 0).retry(delay, reason, backupUrl) //retry stream1
 *
 * NOTE:
 * If you call this methods nothing is executed:
 *
 * multiCamera1.startStream(endpoint);
 * multiCamera1.stopStream();
 *
 ...
 */
class MultiCamera1: Camera1Base {
