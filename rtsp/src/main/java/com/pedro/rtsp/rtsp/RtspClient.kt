 ...
 */
class RtspClient(val connectCheckerRtsp: ConnectCheckerRtsp) {
            ...
            thread = Thread(object : Runnable {
                override fun run() {
                    try {
                        ...
                        val response = commandsManager.getResponse(reader, connectCheckerRtsp, false, false)
                                ?: ""
                        ...
                        if (status == 403) {
                            ...
                            if (commandsManager.user == null || commandsManager.password == null) {
                                ...
                                writer?.flush()
                                val statusAuth = commandsManager.getResponseStatus(commandsManager.getResponse(reader, connectCheckerRtsp, false, false)
                                        ?: "")
                                when (statusAuth) {
                                    401 -> {
                                        connectCheckerRtsp.onAuthErrorRtsp()
