...
class CommandsManager {
    ...
    private fun addHeaders(): String {
     ...
     """.trimIndent() + (if (authorization != null) """
    ...
    private fun createBody(): String {
        var videoBody = ""
        if (!isOnlyAudio) {
            videoBody = if (vps == null) createH264Body(trackVideo, spsString, ppsString) else createH265Body(trackVideo, spsString, ppsString, vpsString)
        }
        return """v=0
...
${addHeaders()}"""
    ...
    fun createSetup(track: Int): String {
        ...
        val params = if (protocol === Protocol.UDP) "UDP;unicast;client_port=" + udpPorts[0] + "-" + udpPorts[1] + ";mode=record" else "TCP;interleaved=" + 2 * track + "-" + (2 * track + 1) + ";mode=record"
