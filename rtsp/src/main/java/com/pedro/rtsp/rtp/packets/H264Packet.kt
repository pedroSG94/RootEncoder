 ...
 * RFC 3984
 */
class H264Packet(sps: ByteArray, pps: ByteArray, private val videoPacketCallback: VideoPacketCallback) : BasePacket(RtpConstants.clockVideoFrequency) {
    ...
    override fun createAndSendPacket(byteBuffer: ByteBuffer, bufferInfo: MediaCodec.BufferInfo) {
        ...
        if (type == RtpConstants.IDR || bufferInfo.flags == MediaCodec.BUFFER_FLAG_KEY_FRAME) {
            stapA?.let {
                val buffer = getBuffer(it.size ?: 0 + RtpConstants.RTP_HEADER_LENGTH)
        ...
        if (sendKeyFrame) {
            ...
            if (naluLength <= maxPacketSize - RtpConstants.RTP_HEADER_LENGTH - 2) {
                ...
                while (sum < naluLength) {
                    ...
                    updateSeq(buffer)
                    val rtpFrame = RtpFrame(buffer, ts, length + RtpConstants.RTP_HEADER_LENGTH + 2, rtpPort,
                            rtcpPort, channelIdentifier)
                    videoPacketCallback.onVideoFrameCreated(rtpFrame)
                    // Switch start bit
                    header[1] = (header[1] and 0x7F) as Byte
                }
            }
        }
    }
