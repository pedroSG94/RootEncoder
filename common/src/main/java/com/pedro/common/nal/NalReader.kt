...
object NalReader {
  ...
  fun extractNals(buffer: ByteBuffer, codec: VideoCodec, shouldDiscardVideoInfo: Boolean): ArrayList<ByteBuffer> {
    ...
    units.removeAll { shouldDiscardNal(it, codec, shouldDiscardVideoInfo) }
    if (units.isEmpty()) units.add(buffer)
    return units
  }

  private fun shouldDiscardNal(nal: ByteBuffer, codec: VideoCodec, shouldDiscardVideoInfo: Boolean): Boolean {
    return when (codec) {
      VideoCodec.H264 -> {
