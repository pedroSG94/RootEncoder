...
public class AndroidViewFilterRender extends BaseFilterRender {
    ...
    thread.execute(() -> {
      while (running) {
        ...
        if (status == Status.RENDER1 || status == Status.RENDER2) {
          ...
          View view = this.view;
          if (view == null) {
            unlockCanvasAndPost(status, surface, surface2, canvas);
            continue;
          }
          ...
          try {
            ...
            mainHandler.post(() -> {
              View mainThreadView = this.view;
              if (mainThreadView != null) {
                mainThreadView.draw(canvas);
              }
              unlockCanvasAndPost(status, surface, surface2, canvas);
            });
          }
  ...
  private void unlockCanvasAndPost(Status status, Surface surface, Surface surface2, Canvas canvas) {
