...
public class ImageObjectFilterRender extends BaseFilterRender {
  ...
  public ImageObjectFilterRender() {
    imageStreamObject = new ImageStreamObject();
  ...
  public void release() {
    ...
    sprite.reset();
    if (imageStreamObject != null) imageStreamObject.recycle();
  }
  ...
  public void setDefaultScale(int streamWidth, int streamHeight) {
    sprite.scale(imageStreamObject.getWidth() * 100 / streamWidth,
        imageStreamObject.getHeight() * 100 / streamHeight);
    squareVertexWatermark.put(sprite.getTransformedVertices()).position(0);
  }
}
